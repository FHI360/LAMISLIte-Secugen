...
//    public String CheckIfFingerAlreadyCaptured(List<Biometric> fingerPrint) {
//        try {
//            int index = 0;
//            while (index < fingerPrint.size()) {
//                List<Biometric> compare = new ArrayList<>(fingerPrint);
...
//    }
    ...
    public String CheckIfFingerAlreadyCaptured(String newTemplate, List<Biometric> compare) {
        ...
        try {
            ...
            for (Biometric each : compare) {
                int[] matchScore = new int[1];
                if (each.getTemplate() != null) {

                    debugMessage("Checking against : " + each.getPatient().getId() + " finger: " + each.getTemplateType().name());
                    ...
                    if (iError == SGFDxErrorCode.SGFDX_ERROR_NONE) {
                        if (matched[0]) {
                            ...
                            debugMessage("found match : " + each.getTemplateType() + " score - " + matchScore[0]);
                            return decodeFingerPosition(each.getTemplateType().name());
                        }
                    }
                }
            }
...
//    public String CheckIfFingerAlreadyCaptured(byte[] newTemplate, List<Biometric> compare) {
//        if (compare == null || compare.size() == 0) return null;
//
//        boolean[] matched = new boolean[1];
//        try {
//            byte[] unknownTemplateArray = newTemplate;
//
//            for (Biometric each : compare) {
//                int[] matchScore = new int[1];
//                if (each.getTemplate() != null) {
//                    debugMessage("Checking against : " + each.getPatient().getId() + " finger: " + each.getTemplateType().name());
//
//                    byte[] fingerTemplate = each.getTemplate();
//                    long iError = sgfplib.MatchIsoTemplate(fingerTemplate, 0, unknownTemplateArray, 0, SGFDxSecurityLevel.SL_ABOVE_NORMAL, matched);
//                    if (iError == SGFDxErrorCode.SGFDX_ERROR_NONE) {
//                        if (matched[0]) {
//                            sgfplib.GetIsoMatchingScore(fingerTemplate, 0, unknownTemplateArray, 0, matchScore);
//                            debugMessage("found match : " + each.getTemplateType().name() + " score - " + matchScore[0]);
//                            return decodeFingerPosition(each.getTemplateType().name());
//                        }
//                    }
//                }
